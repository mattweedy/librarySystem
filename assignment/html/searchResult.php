    ...
    <body>
        ...
        <div class="search-box">
            <form action="searchResult.php" method="GET" class="search-form">
                ...
                <a href="reservedBooks.php" id='reserved_books'>View Your Reserved Books</a> |
                <a href="library.php" id='reserved_books'>View All Books</a>
            </form>
        </div>
        <div class="main">
            <?php
            // getting user input for search and cleaning it
            ...
            $num_result = mysqli_query($conn, "SELECT COUNT(*) AS total_search_results FROM books WHERE books.BookTitle LIKE '%$search%' or books.Author LIKE '%$search%'");
            if ($categories == "NULL") {
                $sql = "SELECT
                            books.ISBN,
                            books.BookTitle,
                        ...
                        WHERE
                            books.Author LIKE '%$search%' OR books.BookTitle LIKE '%$search%'
                        LIMIT 5 OFFSET 0;";
                ...
                $sql = "SELECT
                        ...
                        INNER JOIN category ON books.Category = category.CategoryID
                        WHERE
                            category.categoryDesc = '$categories'
                        LIMIT 5 OFFSET 0;";
            } else {
                $sql = "SELECT
                            books.ISBN,
                        ...
                        INNER JOIN category ON books.Category = category.CategoryID
                        WHERE
                            category.categoryDesc = '$categories' AND books.Author LIKE '%$search%' OR books.BookTitle LIKE '%$search%'
                        LIMIT 5 OFFSET 0;";
            }
            // echo "$sql<br>";
            $total_records = mysqli_fetch_array($num_result);
            $total_records = $total_records['total_search_results'];
